...
interface FeatureHeaderProps {
  title: string;
  subFeatures: { name: string; href: string }[];
  activeSubFeature: string | null;
  onMenuClick: () => void;
}

const FeatureHeader: FC<FeatureHeaderProps> = ({ title, subFeatures, activeSubFeature, onMenuClick }) => {
  return (
    // This is a single-row header, which is cleaner and more professional for responsive design.
    <header className="flex-shrink-0 flex flex-col p-2 ">
          ...
          {subFeatures.map((feature) => (
            <Link href={feature.href} key={feature.name}>
              <span className={`py-2 px-1 text-sm font-medium cursor-pointer transition-colors whitespace-nowrap border-b-2 
                ${activeSubFeature === feature.name.toLowerCase()
                  ? 'text-primary border-primary' // Correct active state with underline
                  : 'text-secondary hover:text-white border-transparent' // Inactive state
              }`}>
                {feature.name}
              </span>
            </Link>
      ...
      {/* This entire nav is hidden on large screens */}
