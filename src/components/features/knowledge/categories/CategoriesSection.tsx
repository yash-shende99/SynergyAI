...
export default function CategoriesSection({ projectId }: CategoriesSectionProps) {
  ...
  const fetchDocuments = async (category: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/knowledge/documents/category/${encodeURIComponent(apiCategory)}`,
        {
          headers: { 'Authorization': `Bearer ${session.access_token}` }
        }
      );
  ...
  const handleDocumentDelete = async (documentId: string) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/knowledge/documents/${documentId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${session.access_token}` }
      });
  ...
  const handleDownloadDocument = async (document: DocumentType) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/knowledge/documents/${document.id}/download`, {
        headers: { 'Authorization': `Bearer ${session.access_token}` }
      });
