...
export default function UploadSection({ projectId }: UploadSectionProps) {
  ...
  const fetchRecentUploads = useCallback(async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/knowledge/documents`, {
  ...
  const handleDownloadDocument = async (file: UploadedFile) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/knowledge/documents/${file.id}/download`, {
        headers: { 'Authorization': `Bearer ${session.access_token}` }
      });
