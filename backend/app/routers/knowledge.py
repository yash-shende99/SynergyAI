...
async def upload_document_metadata(doc: DocumentCreate, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        result = supabase.table('knowledge_library_documents').insert(insert_data).execute()
        ...
        supabase.table('knowledge_library_documents').update(update_data).eq('id', new_document['id']).execute()
...
@router.get("/api/knowledge/categories-list")
async def get_categories_list( user_id: str = Depends(get_current_user_id)):
    """Get list of available categories"""
    try:
        result = supabase.table('knowledge_library_documents').select('category').not_.is_('category', None).not_.eq('category', '').execute()
...
async def get_knowledge_library_documents( user_id: str = Depends(get_current_user_id)):
    """Fetches the most recent VDR documents for a given project."""
    try:
        result = supabase.table('knowledge_library_documents').select('*').eq('uploaded_by_user_id', user_id).order('uploaded_at', desc=True).limit(10).execute()
        return result.data
    except Exception as e:
...
async def upload_document(
    ...
    try:
        ...
        project_dir = UPLOAD_DIR / user_id
...
@router.get("/api/knowledge/documents/{document_id}/download")
async def download_document(document_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        result = supabase.table('knowledge_library_documents').select('*').eq('id', document_id).execute()
        if not result.data:
        ...
        with open('download_debug.log', 'a') as f:
            f.write(f"Downloading {document_id}: mime_type={mime_type}, path={file_path}, exists={file_path.exists()}\n")
...
async def get_categories( user_id: str = Depends(get_current_user_id)):
    ...
    try:
        # Fetch all documents for the user to calculate counts
        result = supabase.table('knowledge_library_documents').select('category').eq('uploaded_by_user_id', user_id).execute()
        ...
        # Format for response
        response_data = [{'name': k, 'document_count': v} for k, v in category_counts.items()]
        
        # Sort so predefined ones are first, or alphabetically
        ...
        return [
            ...
            {'name': 'General', 'document_count': 0},
            {'name': 'Uncategorized', 'document_count': 0}
        ]

@router.get("/api/knowledge/documents/category/{category}")
async def get_documents_by_category( category: str, user_id: str = Depends(get_current_user_id)):
    """Get VDR documents by category (including uncategorized)."""
    try:
        if category.lower() in ['uncategorized', 'null', 'none', '']:
            result = supabase.table('knowledge_library_documents').select('*').eq('uploaded_by_user_id', user_id).or_(f'category.is.null,category.eq.""').order('uploaded_at', desc=True).execute()
        else:
            result = supabase.table('knowledge_library_documents').select('*').eq('uploaded_by_user_id', user_id).eq('category', category).order('uploaded_at', desc=True).execute()
        return result.data
    except Exception as e:
...
async def delete_document(document_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        ...
        supabase.table('knowledge_library_documents').delete().eq('id', document_id).execute()
...
@router.get("/api/knowledge/documents/{document_id}/preview")
...
async def vdr_search( search_query: VdrSearchQuery, user_id: str = Depends(get_current_user_id)):
    """Performs a search scoped ONLY to the documents within a specific project's VDR."""
    try:
        docs_res = supabase.table('knowledge_library_documents').select('id, file_name').eq('uploaded_by_user_id', user_id).execute()
        filename_to_id = {doc['file_name']: doc['id'] for doc in docs_res.data}
        allowed_filenames = list(filename_to_id.keys())
        ...
        if search_query.mode == 'semantic':
            ...
            for chunk in context_chunks:
                highlighted_excerpt = re.sub(f"(?i)({re.escape(search_query.query)})", r"<mark>\1</mark>", chunk['content'])
                doc_id = filename_to_id.get(chunk['source'])
                results.append({ "id": doc_id, "docName": chunk['source'], "excerpt": f"...{highlighted_excerpt}..." })
            
            # Fail-safe: if semantic post-filtering crowded out our results but the word actually exists!
            if not results:
                query_lower = search_query.query.lower()
                for chunk in rag_system.chunk_map:
                    if chunk['source'] in allowed_filenames and query_lower in chunk['content'].lower():
                        idx = chunk['content'].lower().find(query_lower)
                        start = max(0, idx - 40)
                        end = min(len(chunk['content']), idx + len(query_lower) + 40)
                        highlighted_excerpt = re.sub(f"(?i)({re.escape(search_query.query)})", r"<mark>\1</mark>", chunk['content'][start:end])
                        results.append({ "id": filename_to_id.get(chunk['source']), "docName": chunk['source'], "excerpt": f"...{highlighted_excerpt}..." })
            ...
            import re
            for chunk in rag_system.chunk_map:
                if chunk['source'] in allowed_filenames:
                    if query_lower in chunk['content'].lower():
                        idx = chunk['content'].lower().find(query_lower)
                        start = max(0, idx - 40)
                        end = min(len(chunk['content']), idx + len(query_lower) + 40)
                        excerpt_raw = chunk['content'][start:end]
                        highlighted_excerpt = re.sub(f"(?i)({re.escape(search_query.query)})", r"<mark>\1</mark>", excerpt_raw)
                        doc_id = filename_to_id.get(chunk['source'])
                        results.append({ "id": doc_id, "docName": chunk['source'], "excerpt": f"...{highlighted_excerpt}..." })
                        if len(results) >= 10:
                            break
            ...
            if not results:
                fulltext_res = supabase.table('knowledge_library_documents').select('id, file_name').eq('uploaded_by_user_id', user_id).ilike('file_name', f"%{search_query.query}%").limit(10).execute()
                results = [{ "id": doc['id'], "docName": doc['file_name'], "excerpt": "Keyword match found in document title." } for doc in fulltext_res.data]
...
@router.get("/api/knowledge/chat")
async def get_vdr_chat_history( user_id: str = Depends(get_current_user_id)):
    """Get VDR QA history for this project."""
    try:
        result = supabase.table('vdr_qa_sessions').select('id, messages').limit(1).single().execute()
        if not result.data:
...
async def vdr_qa_and_save( query: VDRQuery, user_id: str = Depends(get_current_user_id)):
    """Perform Q&A scoped to this project's VDR, saving the interaction."""
    try:
        docs_res = supabase.table('knowledge_library_documents').select('id, file_name').execute()
...
@router.get("/api/knowledge/annotated_documents", response_model=List[Dict])
async def get_annotated_documents( user_id: str = Depends(get_current_user_id)):
    """Fetches ALL documents for the project with annotation counts."""
    try:
        docs_result = supabase.table('knowledge_library_documents')\
...
async def delete_annotation(annotation_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        anno_res = supabase.table('document_annotations').select('created_by_user_id, user_id').eq('id', annotation_id).single().execute()
        if not anno_res.data:
            raise HTTPException(status_code=404, detail="Annotation not found")
        ...
        if anno_res.data['created_by_user_id'] != user_id:
            admin_check = supabase.table('project_members')\
                .select('role')\
                .eq('user_id', anno_res.data['user_id'])\
                .eq('user_id', user_id)\
                .eq('role', 'Admin')\
                .execute()
...
async def get_ai_annotation_suggestions(document_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        doc_res = supabase.table('knowledge_library_documents').select('file_name, file_path').eq('id', document_id).single().execute()
        if not doc_res.data:
            raise HTTPException(status_code=404, detail="Document not found")
...
async def get_document_content(document_id: str, user_id: str = Depends(get_current_user_id)):
    ...
    try:
        doc_res = supabase.table('knowledge_library_documents').select('*').eq('id', document_id).single().execute()
        if not doc_res.data:
            raise HTTPException(status_code=404, detail="Document not found")
